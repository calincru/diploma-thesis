...
hierarchy of predefined \textbf{tables} and \textbf{chains}
(\labelindexref{Figure}{fig:iptables-hierarchy}).  The latter can be further
extended by users, as we will shortly see.
...
\begin{figure}[h]
  ...
  same user-defined chain, it is not very common.}
  \label{fig:iptables-hierarchy}
\end{figure}

\paragraph{Tables.}
Informally, \textbf{tables} are used to classify the rules
based on their purpose.  Thus, each table is introduced to serve a specific
purpose.  The most commonly used ones are:
\begin{itemize}
  \item \textbf{filter} - used for packet filtering only
  \item \textbf{nat} - used for one-to-many NAT, port forwarding, masquerading,
    packet redirecting; it is discussed in more detail in
    \labelindexref{Section}{sub-sec:nat-table}.
  \item \textbf{mangle} - used for specialized packet alteration; one commonly
    used feature is \emph{packet marking}, which allows customized traffic
    handling by the rules following it in the processing stack.
\end{itemize}
...
corresponds to a new hook.  netfilter uses a hook system to allow seamless
extension and hot-plugging.  Therefore, tables are traversed in a
pre-established, built-in \textbf{order}: raw, mangle, nat, filter.

\paragraph{Chains.}
As far as \textbf{chains} are concerned, they specify the point in the
processing stack where the \emph{chain of rules} they represent will be
applied (\labelindexref{Figure}{fig:iptables-organization}).

\begin{figure}[h]
  \centering
  \captionsetup{justification=centering}
  \includegraphics[scale=0.5]{src/img/iptables-organization}
  \caption{A flowgraph showing the usual processing stack in
  netfilter/iptables.}
  \label{fig:iptables-organization}
\end{figure}

\todo{built-in/user-defined}

\paragraph{Rules.}
\todo{Mention its extensions-based design and briefly discuss the two types of
extensions and give some examples.}

\todo{don't make these paragraphs; itemize or smth}
...
\subsection{The \emph{nat} table}\label{sub-sec:nat-table}
