...
\paragraph{Chains.}
As far as the chains are concerned, they specify the point in the processing
stack where the \emph{chain of rules} they represent will be applied
(\labelindexref{Figure}{fig:iptables-organization}).  There are 5
...
\begin{figure}[h]
  ...
  on a yellow background is meaningful only for locally delivered or locally
...
\paragraph{Rules.}
Tables and chains are only meant to organize rules that are essentially
\textbf{(match, action)} pairs.  The first part, the \textbf{match}, is a
...
resulting grammar is shown in \labelindexref{Listing}{lst:rule-grammar}.
...
\subsection{Relation to \emph{conntrack}}
...
chains.  In fact, one of the purposes of the raw table is to allow skipping
connection tracking logic for certain traffic (i.e.
